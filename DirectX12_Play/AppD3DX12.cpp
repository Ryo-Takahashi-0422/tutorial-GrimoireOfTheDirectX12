...
void AppD3DX12::EnableDebugLayer() {
...
std::string AppD3DX12::GetTexPathFromModeAndTexlPath
(
...
// @param str マルチバイト文字列
// return 変換されたワイド文字列
std::wstring AppD3DX12::GetWideStringFromSring(const std::string& str)
{
	//呼び出し一回目(文字列数を得る)
...
// @param path 対象パス文字列
// @return 拡張子
std::string AppD3DX12::GetExtension(const std::string& path)
{
	int idx = path.rfind('.');
...
// @param splitter 文字列
// @return 分離前後の文字列ペア
std::pair<std::string, std::string> AppD3DX12::SplitFileName(
...
AppD3DX12::LoadTextureFromFile(TexMetadata* metaData, Image* img, std::string& texPath)
{
...
ComPtr<ID3D12Resource> AppD3DX12::CreateMappedSphSpaTexResource(TexMetadata* metaData, Image* img, std::string texPath)
{
...
ComPtr<ID3D12Resource> AppD3DX12::CreateColorTexture(const int param)
{
...
ComPtr<ID3D12Resource> AppD3DX12::CreateGrayGradationTexture()
{
...
//初期化処理６：フレームリソース(各フレームのレンダーターゲットビュー)を作成
	std::vector<ComPtr<ID3D12Resource>> _backBuffers(swapChainDesc.BufferCount);//リソースバッファー
	...
	std::vector<ComPtr<ID3D12Resource>> texUploadBuff(materialNum);//テクスチャCPUアップロード用バッファー
	std::vector<ComPtr<ID3D12Resource>> texReadBuff(materialNum);//テクスチャGPU読み取り用バッファー
	std::vector<ComPtr<ID3D12Resource>> sphMappedBuff(materialNum);//sph用バッファー
	std::vector<ComPtr<ID3D12Resource>> spaMappedBuff(materialNum);//spa用バッファー
	std::vector<ComPtr<ID3D12Resource>> toonUploadBuff(materialNum);//トゥーン用アップロードバッファー
	std::vector<ComPtr<ID3D12Resource>> toonReadBuff(materialNum);//トゥーン用リードバッファー
